...
const checks = ['<!DOCTYPE html>', '<html', '</html>', '</body>'];
checks.forEach(c => { if (content.indexOf(c) < 0) console.log('ERROR: missing', c); });

// 1. Replace renderShenboTree function
// Find the function body between { and the next function
const s2fn = content.indexOf('function renderShenboTree()');
const s2brace = content.indexOf('{', s2fn);
const d2fn = content.indexOf('function renderDongshanTree()');
if (s2fn < 0 || s2brace < 0 || d2fn < 0) { console.log('ERROR: boundaries'); process.exit(1); }

// The renderShenboTree function body starts after the { and ends before renderDongshanTree
const s2body = content.substring(s2brace + 1, d2fn);
// Find the matching closing brace at the end of the function
const s2closing = s2body.lastIndexOf('\n      }');
if (s2closing < 0) { console.log('ERROR: s2 closing brace'); process.exit(1); }
...
content = content.substring(0, s2brace + 1) + s2newBody + content.substring(d2fn);

// 2. Replace renderDongshanTree function
const d2fn2 = content.indexOf('function renderDongshanTree()');
const d2brace = content.indexOf('{', d2fn2);
const reexport = content.indexOf('// Re-export render');
if (d2fn2 < 0 || d2brace < 0 || reexport < 0) { console.log('ERROR: d2 boundaries'); process.exit(1); }

const d2body = content.substring(d2brace + 1, reexport);
const d2closing = d2body.lastIndexOf('\n      }');
if (d2closing < 0) { console.log('ERROR: d2 closing'); process.exit(1); }
...
// Find the exact end of the function - it's the closing } before '// Re-export'
const d2ActualEnd = reexport;
const d2EndInBody = d2body.lastIndexOf('\n      }');
const d2End = d2brace + 1 + d2EndInBody + 1;

content = content.substring(0, d2brace + 1) + d2newBody + content.substring(d2End);
      ...
      function renderChainTable(gYD, gSB, nms, nts) {
        ...
        for (var i=0; i<nms.length; i++) {
          ...
          if (i === 0) {
            ...
            html += '<div style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;border:1px solid var(--glass-border);background:var(--bg-card);cursor:pointer;margin-left:28px;margin-top:1px;" onclick="lineageDetail('+(findPId(nms[i])||0)+')">';
...
if (content.indexOf('function renderChainTable') < 0) {
  content = content.substring(0, rcStart) + tableFn + '\n\n' + content.substring(rcStart);
}

// Final verification
console.log('DOCTYPE:', (content.match(/<!DOCTYPE>/g)||[]).length);
console.log('Has renderShenboTree:', content.includes('function renderShenboTree'));
console.log('Has renderDongshanTree:', content.includes('function renderDongshanTree'));
console.log('Has renderChainTable:', content.includes('function renderChainTable'));
console.log('Has renderChain:', content.includes('function renderChain'));
