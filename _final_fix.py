...
assert content.count('<!DOCTYPE html>') == 1, f"DOCTYPE count: {content.count('<!DOCTYPE html>')}"
assert content.count('</html>') == 1, f"/html count: {content.count('</html>')}"

# ==================== 1. Find and replace Shenbo function body ====================
s2_marker = 'function renderShenboTree()'
d2_marker = 'function renderDongshanTree()'
rc_marker = 'function renderChain('

assert s2_marker in content, "s2 marker not found"
assert d2_marker in content, "d2 marker not found"
assert rc_marker in content, "rc marker not found"

# Find shenbo function body boundaries
s2_fn_start = content.index(s2_marker)
s2_brace = content.index('{', s2_fn_start)
# The function ends just before dongshan function
s2_between = content[s2_brace:s2_brace+2000]
# Find the LAST '}' before renderDongshanTree
d2_start = content.index(d2_marker)
s2_closing = content.rindex('\n      }', s2_brace, d2_start)
s2_close = s2_closing + 8  # include the }

print(f"Shenbo: fn_start={s2_fn_start}, brace={s2_brace}, close={s2_close}")
print(f"  Shenbo body length: {s2_close - s2_brace}")

# New shenbo function body
s2_new = """ {
...
# ==================== 2. Find and replace Dongshan function body ====================
d2_fn_start = content.index(d2_marker)
d2_brace = content.index('{', d2_fn_start)
re_marker = 'window.renderAllLineageTrees'
assert re_marker in content, "re-export not found"
re_start = content.index(re_marker, d2_brace)
# Find the LAST closing brace before re-export
d2_section = content[d2_brace:re_start]
d2_closing = d2_section.rindex('\n      }')
d2_close = d2_brace + d2_closing + 8

print(f"Dongshan: fn_start={d2_fn_start}, brace={d2_brace}, close={d2_close}")
...
# ==================== 3. Add renderChainTable function ====================
if 'function renderChainTable' not in content:
    rc_pos = content.index(rc_marker)
    table_fn = """
      // ===== 表格链渲染器 =====
...
"""
    content = content[:rc_pos] + table_fn + '\n\n' + content[rc_pos:]

# ==================== 4. Verify ====================
print('DOCTYPE:', content.count('<!DOCTYPE html>'))
print('/html:', content.count('</html>'))
print('/body:', content.count('</body>'))
...
assert content.count('<!DOCTYPE html>') == 1, "Structure corrupted!"
assert content.count('</html>') == 1, "Missing /html!"
assert content.count('function renderChainTable') == 1, "renderChainTable missing!"
