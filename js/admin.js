...
const MODULES = {
  news: {
    ...
    fields: [
      ...
      { key: 'category', label: '分类', type: 'select', options: ['家族事务', '通知公告', '家族活动', '节日祝福', '投稿专栏', '网站公告'], required: true },
      { key: 'date', label: '日期', type: 'date', required: true },
      { key: 'author', label: '发布人', type: 'text', required: true },
      { key: 'url', label: '原文链接', type: 'text', placeholder: '可粘贴原文网址（选填）' },
  ...
  videos: {
    ...
    fields: [
      ...
      { key: 'poster', label: '封面图标', type: 'select', options: ['🎬', '📹', '🎥', '📽️'] },
      { key: 'desc', label: '简介', type: 'textarea' }
    ],
    defaultData: [
      { id: 1, title: '下枫槎谢氏家族宣传片', url: '', embed: '', poster: '🎬', desc: '下枫槎村谢氏家族数字宗祠宣传片', hasFile: false }
    ]
  },
  messages: {
    label: '留言管理',
...
function renderModule(mod) {
  ...
  if (data.length === 0) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      html += '<tr>';
      m.fields.forEach(function(f) {
        if (f.key === 'content' || f.key === 'message' || f.key === 'file') return;
        var val = item[f.key] || '—';
        if (f.key === 'icon' || f.key === 'avatar') {
          html += '<td style="font-size:20px;">' + val + '</td>';
        } else if (f.key === 'title' && (mod === 'photos' || mod === 'templeCarousel') && (item.hasFile || item.file_url)) {
          // Show thumbnail for photos and carousel
          var thumbSrc = item.file_url || '';
          html += '<td><div style="display:flex;align-items:center;gap:8px;">';
          if (thumbSrc) {
            html += '<img src="' + thumbSrc + '" alt="" style="width:48px;height:48px;border-radius:6px;object-fit:cover;flex-shrink:0;">';
          } else {
            html += '<span style="width:48px;height:48px;border-radius:6px;overflow:hidden;background:var(--bg-card);display:inline-flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;">📷</span>';
          }
...
function saveForm(mod, editId) {
  ...
  // Handle file upload — upload to server, store URL in metadata
  if (fileInput && fileInput.files && fileInput.files[0]) {
    var file = fileInput.files[0];
    var prefix = mod === 'photos' ? 'photo_' : (mod === 'videos' ? 'video_' : (mod === 'templeCarousel' ? 'carousel_' : 'file_'));

    uploadToServer(prefix + idToSave, file).then(function(result) {
      if (result && result.url) {
        for (var idx = 0; idx < data.length; idx++) {
          if (data[idx].id === idToSave) {
            data[idx].file_url = result.url;
            data[idx].fileName = file.name;
            break;
          }
        }
        saveData(mod, data);
      } else {
        console.error('File upload failed — server returned:', JSON.stringify(result));
        var errDetail = result && result.error ? result.error : '服务器返回格式异常';
        alert('文件上传失败: ' + errDetail);
      }
    }).catch(function(err) {
      console.error('Upload failed:', err);
      alert('文件上传失败: ' + (err.message || '网络错误'));
    });
  }
...
function renderSettings(area) {
  ...
  // Update counts
  document.getElementById('settings-news-count').textContent = getData('news').length;
