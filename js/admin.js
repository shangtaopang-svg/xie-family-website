...
function renderGenealogy(area) {
  ...
  // Merge hardcoded ancient lineage data so 全部世代总览 tree shows everyone
  var allData = data.concat(getShenboData()).concat(getDongshanData()).concat(getLinhaiData()).concat(getShimaData());
  // Dedup by ID
  var seen = {};
  allData = allData.filter(function(p) { if (seen[p.id]) return false; seen[p.id] = true; return true; });
...
function renderGenealogyTree() {
  ...
  if (!treeEl) return;
  var allData = getData('genealogy').concat(getShenboData()).concat(getDongshanData()).concat(getLinhaiData()).concat(getShimaData());
  var seen = {};
  allData = allData.filter(function(p) { if (seen[p.id]) return false; seen[p.id] = true; return true; });
