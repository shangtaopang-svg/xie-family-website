...
assert content.count('<!DOCTYPE html>') == 1, "Multiple DOCTYPE!"

# 1. Replace renderShenboTree function
s2_start = content.find('function renderShenboTree()')
s2_body_start = content.find('{', s2_start)
s2_body_end = content.find('function renderDongshanTree', s2_start)
s2_close = content.rfind('\n      }', s2_body_start, s2_body_end) + 8
...
s2_new = """ {
        ...
        var nms = ['申伯','弘','猛','广','协','列宗','穆宗','高','预','昌后','达','守礼','子民','秩','頴','林','浣','旺','玽','国辉','宁','福','杨贞','平利','平和','平祖','翠','利','文','武','秉槐','堂','瑛','文轩','文昂','福郎','丙郎','应郎','宜礼','宜乐','逢','简','瑰','懿','鲅','当','景秀','缵','显','顆'];
        var nts = ['谢氏鼻祖','申伯之子','申伯之子','弘之子','弘之子','广之子','广之子','列宗之子','高之子','预之子','昌后之子','昌后之子','达之子','子民之子','秩之子','頴之子','林之子','浣之子','旺之子','玽之子','国辉之子','宁之子','福之子','杨贞之子','杨贞之子','杨贞之子','平利之子','平利之子','平和之子','文之子','武之子','秉槐之子','堂之子','瑛之子','瑛之子','文轩之子','文轩之子','文轩之子','福郎之子','福郎之子','宜礼之子','逢之子','简之子','瑰之子','懿之子','鲅之后','鲅之后','景秀之后','景秀之后','景秀之后'];
        var html = renderChainTable(gYD, gSB, nms, nts);
        document.getElementById('tree-shenbo').innerHTML = html;
      }"""

content = content[:s2_body_start] + s2_new + content[s2_close:]

# 2. Replace renderDongshanTree function
d2_start = content.find('function renderDongshanTree()')
d2_body_start = content.find('{', d2_start) + 1
d2_end_marker = content.find('// Re-export render', d2_start)
d2_close = content.rfind('\n      }', d2_body_start, d2_end_marker) + 8
...
d2_new = """ {
        ...
        var nms = ['缵','衡','鲲','裔','广','奕','据','安','万','淮','石','铁','瑶','瑰','肇','峻','混','密','庄','飏','胜','灟','丛','沦','览','琢','侨','琂','琬','琉','峠','植','钝','缪','修','豹','悯','骢','驼','绰','式','革','造','直','是温','羽','静','观','闓'];
        var nts = ['东山第一世','会稽东山始祖','衡之子','衡之子，谢安之父','衡之子','裔之子','裔之子','湄水之战名相','裔之子','裔之子','裔之子','裔之子','安之子','安之子','瑰之子','瑰之子','瑰之子','混之子','密之子','庄之子','庄之子','庄之子','庄之子','庄之子','飏之子','览之子','览之子','琢之子','琢之子','琢之子','琂之子','琂之子','植之子','植之子','钝之子','钝之子','修之子','悯之子','悯之子','悯之子','绰之子','式之子','式之子','造之子','直之子','是温之子','羽之子','羽之子','观之子/临海下渡第一世'];
        var html = renderChainTable(gYD, gSB, nms, nts);
        document.getElementById('tree-dongshan').innerHTML = html;
      }"""

content = content[:d2_body_start-1] + d2_new + content[d2_close:]

# 3. Add renderChainTable function before renderChain
rc_start = content.find('function renderChain(')
    ...
    table_fn = """
      ...
      function renderChainTable(gYD, gSB, nms, nts) {
        ...
        for (var i=0; i<nms.length; i++) {
          ...
          if (i === 0) {
            html += '<div style="display:flex;align-items:flex-start;gap:8px;padding:6px 12px;border-radius:8px;border:1.5px solid var(--accent-orange);background:rgba(251,146,60,0.1);cursor:pointer;" onclick="lineageDetail(0)">';
            ...
            html += '<div style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;border:1px solid var(--glass-border);background:var(--bg-card);cursor:pointer;margin-left:28px;margin-top:1px;">';
...
"""
    content = content[:rc_start] + table_fn + '\n\n' + content[rc_start:]

# Final verification
print('DOCTYPE:', content.count('<!DOCTYPE html>'))
print('Has renderShenboTree:', 'function renderShenboTree' in content)
print('Has renderDongshanTree:', 'function renderDongshanTree' in content)
print('Has renderChainTable:', 'function renderChainTable' in content)
print('Has renderChain:', 'function renderChain' in content)
